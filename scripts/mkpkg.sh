...
# PLATFORM.H — per-platform compile-time defines, kept in the BP.INC include
# file (a directory-file), not in BP, and reached as `$INCLUDE BP.INC PLATFORM.H`.
# A build artifact (gitignored): on the MVX target it is comment-only, since MVX
# is a builtin compiler define.  Vendor builds (build-udt.sh / build-uv.sh) write
# their own $DEFINE MV + $DEFINE <vendor> into this file instead.
if grep -rlF -- '$INCLUDE BP.INC PLATFORM.H' "$PKG"/BP >/dev/null 2>&1; then
  mkdir -p "$PKG/BP.INC"
  cat > "$PKG/BP.INC/PLATFORM.H" <<'PLATH'

...
*
* GENERATED FILE - DO NOT EDIT.  It is written by mkpkg.sh every time the
...
* for it - VOC everywhere except jBASE, which calls it MD.  An EQU rather than a
* valued $DEFINE: UniData's $DEFINE takes no value at all (it is a flag for
* $IFDEF) and a quoted value fails to compile there, while compiling fine on
* jBASE and MVX - so the portable spelling is this one.  Verified on all four.
      EQU MVMASTER TO "VOC"
PLATH
